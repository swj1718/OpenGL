...
float backgruond_high = 20 * 600;//작업
int shape = 0;
float box_speed = 20;
float right = 0;
float up = 0;
...
GLfloat lightPos[][4] = { { -50, 300, -50, 1 }, { 50, 300, 50, 1 }, { -50, 300, 50, 1 }, { 50, 300, -50, 1 }
}; // 광원의 위치
...
void SetupRC()
{
	...
	glLightfv(GL_LIGHT1, GL_DIFFUSE, diffuseLight);
	glLightfv(GL_LIGHT1, GL_SPECULAR, specular);
	glLightfv(GL_LIGHT1, GL_POSITION, lightPos[3]);
...
void DrawScene(void)
{
	...
	glRotatef(mouse_pos_x*0.3, 0, 1, 0);	//마우스 움직일시 회전 //작업
	...
	glPushMatrix();
	glTranslatef(-1000 + ai_move_x, -200, -3900 + ai_move_y);
	ai();
	glPopMatrix();

	glPushMatrix();
	//마우스 움직일시 회전
	glTranslatef(right, -400 + down + 190, up - 100); // 캐릭터 이동
	glPushMatrix();
	glRotatef(character_mouse_pos_x*0.3, 0, -1, 0);
	character();
	glPopMatrix();
...
void TimerFunction(int value)//작업
{
	...
	ai_move_y += ai_speed_y;
	if (ai_move_y > 1200)
	{
		ai_speed_y = 0;
		ai_move_x += ai_speed_x;
		if (ai_move_x > 2000)
		{
			ai_speed_x *= -1;
		}
	...
	distance = sqrt(((ai_move_x - 1000) - right)* ((ai_move_x - 1000) - right) + ((ai_move_y - 3900) - (up - 100))* ((ai_move_y - 3900) - (up - 100)));
	...
	if (right <= -3450)//왼쪽 끝 한줄짜리 벽
	{
		right = -3450;
	}
	if (right >= 3450)//오른쪽 끝 한줄짜리 벽
	{
		right = 3450;
	}
...
void crash(float x, float y)
{
	if (right>right_point - 1000 * x - 30 + 50 && right<right_point - 1000 * x + 50 && up<down_point - 600 * y && up>up_point - 600 * y)//벽왼쪽
	{
		right = right_point + 50 - 1000 * x;

	}
	if (right<left_point - 1000 * x + 30 - 50 && right>left_point - 1000 * x - 50 && up<down_point - 600 * y && up>up_point - 600 * y)//벽왼쪽
	{
		right = left_point - 50 - 1000 * x;

	}
	if (right<right_point - 1000 * x  && right>left_point - 1000 * x  && up>up_point - 600 * y - 50 && up<up_point - 600 * y + 30 - 50)//벽위쪽
	{
		up = up_point - 50 - 600 * y;

	}
	if (right<right_point - 1000 * x  && right>left_point - 1000 * x   && up<down_point - 600 * y + 50 && up>down_point - 600 * y - 30 + 50)//벽아래쪽
	{
		up = down_point + 50 - 600 * y;
...
GLvoid Motion(int x, int y)
{
	mouse_pos_x = -600 + x;
	mouse_pos_y = 300 - y;
	character_mouse_pos_x = -600 + x;
	character_mouse_pos_y = 300 - y;
	glutPostRedisplay();
}
